...
//author:       wangxuewen
//mobile:       15927593508
//description:  header file of class Decoder
//======================================================
#include "ffdecoder.h"
#include <iostream>
...
bool FFDecoder::open(unsigned char *video_buf, int len)
{
    ...
    io_ctx_buffer = (unsigned char*)av_malloc(io_ctx_buffer_size);
    if (!io_ctx_buffer)
    {
        return false;
    }
...
bool FFDecoder::grab()
{
    ...
    while (av_read_frame(format_ctx, &pkt) >= 0)
    {
        ...
        {
            avcodec_decode_video2(codec_ctx, frame, &finished, &pkt);
            av_packet_unref(&pkt);
            frame_pos += 1;
            break;
        }
